        ...
        <div className="space-y-3">
          {faqs.map((faq, index) => (
            <motion.div 
              ...
              key={index}
              className={`glass-card rounded-2xl border border-white/5 overflow-hidden transition-all duration-300 ${openIndex === index ? 'bg-white/10 border-white/20' : 'bg-transparent hover:bg-white/[0.03]'}`}
            >
              <button
                className="w-full px-5 md:px-6 py-4 md:py-5 text-left flex justify-between items-center bg-transparent gap-4"
                onClick={() => setOpenIndex(openIndex === index ? null : index)}
              >
                <span className="font-bold text-base text-white pr-4 leading-tight">{faq.question}</span>
                <span className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 transition-colors ${openIndex === index ? 'bg-pro-red text-white shadow-lg shadow-pro-red/20' : 'bg-white/5 text-gray-400 group-hover:bg-white/10'}`}>
                ...
                {openIndex === index && (
                  ...
                  >
                    <div className="px-5 md:px-6 pb-5">
                      <p className="text-gray-400 text-sm leading-relaxed font-light pt-3 border-t border-white/5">
                        {faq.answer}
                      </p>
                    </div>
                  </motion.div>
